...
class Destinasi extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        if (!empty($_FILES['destinasi_gambar']['name'])) {
            ...
            $config['upload_path']   = './uploads/destinasi/';
        ...
        $data = [
            ...
            'destinasi_nama'       => $this->input->post('destinasi_nama'),
            'destinasi_deskripsi'  => $this->input->post('destinasi_deskripsi'),
            'destinasi_alamat'     => $this->input->post('destinasi_alamat'),
    ...
    public function update()
    {
        ...
        if (!empty($_FILES['destinasi_gambar']['name'])) {
            ...
            if ($this->upload->do_upload('destinasi_gambar')) {
                ...
                $lama = $this->M_destinasi->get_by_id($id);

                if (
                    $lama &&
                    $lama->destinasi_gambar &&
                    file_exists('./uploads/destinasi/' . $lama->destinasi_gambar)
                ) {
                    unlink('./uploads/destinasi/' . $lama->destinasi_gambar);
                }
    ...
    {
        $data = $this->M_destinasi->get_by_id($id);

        if (
            $data &&
            $data->destinasi_gambar &&
            file_exists('./uploads/destinasi/' . $data->destinasi_gambar)
        ) {
            unlink('./uploads/destinasi/' . $data->destinasi_gambar);
        }
